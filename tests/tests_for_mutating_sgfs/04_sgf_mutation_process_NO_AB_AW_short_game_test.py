...
def test_entire_workflow():
  '''
    This test function executes the entire workflow for SGF string manipulation
    so that Glift can render the puzzles from the new mutated SGF string properly.

    Note: This test does NOT contain automated assertions. To verify the test,
    manually inspect each generated sgf_string using an SGF viewer to ensure they are valid.
    '''
  ...
  # 50 visit analysis on 67 move game took computer's CPU (Eigen) 240 seconds to analyze
  katago_json_output = '''{
    "createdPuzzles": [
        {
            "id": 1,
  ...
  # View each new sgf_string
  for sgf_string in final_sgf_strings:
      print(sgf_string)

# IF Pass is answer, then we get error: ValueError: invalid literal for int() with base 10: 'ass'
# Because it's trying to read from the 1st index (aka 2nd letter) as an integer to convert the coordinate for
